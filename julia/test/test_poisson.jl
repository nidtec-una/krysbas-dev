@testset "Poisson integration" begin
    ...
    @testset "1D Poisson, Dirichlet BC" begin
        ...
        A =
            2 * Matrix(I, INNERNODES, INNERNODES) - diagm(1 => ones(INNERNODES - 1)) -
    ...
    @testset "1D Poisson, Dirichlet-Neumann BC" begin
        ...
        n_sys = INNERNODES + 1
        A =
            2 * Matrix(I, n_sys, n_sys) - diagm(1 => ones(n_sys - 1)) -
    ...
    @testset "1D Poisson, Robin-Robin BC" begin
        ...
        A = diagm(0 => fill(-2.0, NODES), 1 => ones(NODES - 1), -1 => ones(NODES - 1))
        A[1, 1] = h * alpha - 1
        A[end, end] = -h * beta - 1
    ...
    @testset "2D Poisson, Dirichlet BC" begin
        ...
        M =
            4 * Matrix(I, INNODES1D, INNODES1D) - diagm(1 => ones(INNODES1D - 1)) -
        ...
        h2 = h^2
        for i = 1:INNODES1D, j = 1:INNODES1D
